...
El objetivo de esta hoja de trabajo era realizar un analisis exploratorio a la base de datos de peliculas, extraido de IMDB. Es base cuenta con `r nrow(datos)` filas y con `r ncol(datos)` de columnas. <br/>
Para verificar nuestro codigo y lo que hicimos paso por paso puede consultar en nuestro repositorio.<br/>
...
Como se observa en la grafica, del 100% de los ingresos de las peliculas en la base de datos, se demuestra que el `r piepercent[1:1]`% de los ingresos se debe cuando hay mas actores que actrices.
...
Y como se ve en la grafica de pie, se observa que las peliculas con mas hombres que mujeres otienen mayor popularidad. Sin embargo, estos porcentajes son menores a diferencia de los porcentajes de ingresos.
